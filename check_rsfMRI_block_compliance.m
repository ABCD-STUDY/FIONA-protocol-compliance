...
if length(requiredStruct) < index(i)+4  %At least the requested series should be present
    break;
else    
    %First check that the sequence of events is compliant
    testingSeries_FM_1 = requiredStruct(index(i));
    testingSeries_rsfMRI_1 = requiredStruct(index(i)+1);
    testingSeries_rsfMRI_2 = requiredStruct(index(i)+2);
    testingSeries_FM_2 = requiredStruct(index(i)+3);
    testingSeries_rsfMRI_3 = requiredStruct(index(i)+4);
    ...
    if cTypeFinder(testingSeries_rsfMRI_1.fullclassifyType, compliance_key{1,6}.classifyType) &&...,
        ...
        compliance_output.Session_1.rsfMRI_Block.rs_fMRI_FM_1.SeriesInstanceUID = testingSeries_FM_1.SeriesInstanceUID;
        compliance_output.Session_1.rsfMRI_Block.rs_fMRI_FM_1.SeriesNumber = testingSeries_FM_1.SeriesNumber;
        ...
        compliance_output.Session_1.rsfMRI_Block.rs_fMRI_FM_2.SeriesInstanceUID = testingSeries_FM_2.SeriesInstanceUID;
        compliance_output.Session_1.rsfMRI_Block.rs_fMRI_FM_2.SeriesNumber = testingSeries_FM_2.SeriesNumber;
